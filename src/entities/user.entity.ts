...
export class User extends BaseEntity {
  @Column({ unique: true, length: 50 }) nickname: string;
  @Column({ unique: true, length: 100 }) email: string;
  @Column() password: string;
  @Column({ type: "varchar", nullable: true, length: 50 }) name: string | null;
  @Column({ name: "phone_number", type: "varchar", nullable: true, length: 20 })
  phoneNumber: string | null;
  @Column({ type: "varchar", nullable: true, length: 20 }) provider:
    string | null;
