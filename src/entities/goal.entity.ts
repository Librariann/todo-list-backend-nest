...
export class Goal extends BaseEntity {
  ...
  @Column({ name: "target_count", default: 1 }) targetCount: number;
...
export class GoalProcess extends BaseEntity {
  ...
  goal: Goal;
  @Column({ name: "goals_id", type: "bigint" }) goalId: number;
  @ManyToOne(() => User, { nullable: false })
  @JoinColumn({ name: "user_id" })
  user: User;
  @Column({ name: "user_id", type: "bigint" }) userId: number;
  @Column({ name: "period_index", default: 1 }) periodIndex: number;
  ...
  @Column({ name: "is_finalized" }) isFinalized: boolean;
}
