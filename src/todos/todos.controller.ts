...
export class TodosController {
  ...
  ): Promise<ApiResponse<TodoOutput[]>> {
    const result = await this.service.list(user.id, date);
    return success(result, "할 일 목록을 성공적으로 불러왔습니다.");
  }
  @Post("register")
  async create(
    ...
    return success(result, "할 일이 성공적으로 등록 완료되었습니다.");
  }
  ...
  async update(
    ...
    return success(result, "할 일이 성공적으로 수정 완료되었습니다.");
  }
